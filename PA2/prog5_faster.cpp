...
public:
	...
	struct HashIt
	{
		size_t operator()(const CStudent & k)const
		{
			...
			return hash<string>()(transform_name(k.fullName) );
		}
	};
...
public:
	CFilter () {}
	CFilter & Name ( const string & name ) { Data_filter.insert( transform_name(name) ); return *this;}//pridana rovnou transformavane jmeno
	...
	vector<CStudent> operator () ( const unordered_set<CStudent , CStudent::HashIt > & zdroj)const;
private:
...
//----------------------------------------------------------------------------------------------
vector<CStudent> CFilter::operator () ( const unordered_set<CStudent , CStudent::HashIt > & muj_set )const
{
	...
	if( Data_filter.size() != 0 )
	{
		...
		{
			CStudent stud(i, CDate ( 1980, 4, 11), 2010 );
			auto it_start = muj_set.begin( muj_set.bucket(stud) );
			auto it_end = muj_set.end( muj_set.bucket(stud) );
			for( ; it_start != it_end ; it_start++ )
			{
				if( ( Enroll_MIN < it_start->enroll && Enroll_MAX > it_start->enroll )
					&& ( CDate_MAX.CompareTo(it_start->date) == 1 && CDate_MIN.CompareTo(it_start->date) == -1 ) 
					&& i == transform_name(it_start->fullName) )
	...
	else
	{
		for(auto i : muj_set )
		{
			if( ( Enroll_MIN < i.enroll && Enroll_MAX > i.enroll ) && ( CDate_MAX.CompareTo(i.date) == 1 && CDate_MIN.CompareTo(i.date) == -1 ) )
				result.push_back(i);
		}
	}
	return result;
...
{
	vector<CStudent> Found_Students = flt(Students);
